...
- 16 CPUコア / 93GiB RAM / SSD 空き 100GB以上推奨
- ホストユーザが `docker` グループに所属している（`sudo` なしで `docker` を実行可能）
...
```bash
# .env.example を .env にコピーしてパスを調整（出力先・ポート等）
cp .env.example .env
$EDITOR .env
```
...
| 変数 | 意味 | デフォルト |
| --- | --- | --- |
| `SIM_OUTPUT_DIR` | シミュレーション出力先（ローカルSSD推奨） | `${HOME}/gw150914-output` |
| `JUPYTER_PORT` | Jupyter Lab 公開ポート | `8888` |
| `CONTAINER_CPUSET` | 固定する物理CPUコア範囲 | `0-15` |
| `CONTAINER_MEM_LIMIT` | コンテナ最大メモリ | `80g` |
| `CONTAINER_SHM_SIZE` | 共有メモリ（MPI通信用） | `4gb` |

### 2. Docker イメージの取得

```bash
make docker-pull   # einsteintoolkit/jupyter-et を pull
```
...
make docker-token   # Jupyter Lab のアクセスURL(トークン付)を取得
```
...
make docker-check   # ET関連ツール(mpirun, sim)の動作確認
...
| 1 | Docker 環境構築 ([#1](https://github.com/s-sasaki-earthsea-wizard/gw150914-einstein-toolkit/issues/1)) | 🛠️ 環境設定ファイル整備済、動作確認中 |

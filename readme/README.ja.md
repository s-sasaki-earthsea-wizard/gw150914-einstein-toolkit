...
本リポジトリ (parfile generator, Docker 構成, Makefile, テスト, 解析スクリプト
など本リポで作成したもの) は
[GNU General Public License v2.0 or later](../LICENSE)
(`SPDX-License-Identifier: GPL-2.0-or-later`) の下で公開する。

Einstein Toolkit 本体は **本リポでは再配布していない**。Dockerfile が
ビルド時に上流 Bitbucket からチェックアウトする方式で、ET は上流の
ライセンス (各 thorn 単位で GPL/LGPL 等) に従う。詳細は
<https://einsteintoolkit.org> を参照。公式 `GW150914.rpar` パラメータ
ファイルも同様に実行時取得方式で本リポには含めていない。
